...
#-----------------------------
#
#This R script installs the CAOS-R package either from a downloaded
#package file (e.g. caosR_1.0.0.0015.tar.gz) or from github 
#(https://github.com/M0rph3u2x/CAOS-R)
#
#-----------------------------
...
# 0) Locate path to R-Script ---------------------------------------------------

print("0) Automatically locate path to R-Script")
print("")
...
#Install and/or load the package "rstudioapi" via package manager "pacman"
...
#1A) Install/Load CAOS-R package -----------------------------------------------

print("1A) Install/Load CAOS-R package from file (offline")
print("")

caosR <- c("caosR")

# Install caosR package if not yet installed
installed_packages <- caosR %in% rownames(installed.packages())
if (any(installed_packages == FALSE)) {
  caosR_package <- fs::dir_ls("CAOS-R_Package", recurse = TRUE, glob = "*.tar.gz")
  install.packages(FullPath(caosR_package[length(caosR_package)]), repos = NULL, type="source")
}

#-------------------------------------------------------------------------------

#1B) Install/Load CAOS-R package -----------------------------------------------

print("1B) Install/Load CAOS-R package from github (online")
print("")

# Search for packages
packages <- c("devtools","caosR")

# Install CAOS-R if not yet installed
# Source: https://github.com/M0rph3u2x/CAOS-R)
installed_packages <- packages %in% rownames(installed.packages())
if (any(installed_packages == FALSE)) {
  if(installed_packages[1] == FALSE){
    install.packages("devtools")
    library("devtools")
    install_github('M0rph3u2x/CAOS-R')
  }else{
    library("devtools")
    install_github('M0rph3u2x/CAOS-R')
  }
